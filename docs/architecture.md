...
**Refined mechanism.** Prior-vs-context arbitration is capability-dependent and prior-strength-dependent, as demonstrated cleanly by the discriminating stat probe above. Haiku grounds facts where its prior is weak and loses only where its prior is confidently wrong, regardless of prompting; Sonnet overrides even its own confidently wrong priors. The Jaguar case was, at this point in the investigation, read as the sharpest example of that same mechanism. It was not: see "Re-diagnosis" below for why the Jaguar turned out to be a different failure entirely, with the stat probe left standing as the mechanism's real evidence.
