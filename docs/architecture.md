...
The first eval scored generated responses against hand-written ideal answers on two metrics, Faithfulness and Relevance, in a single `judge_response` LLM call on a 1–3 scale across 18 questions. The same call also returned a structured reasoning field alongside the two scores — so reading the judge's rationale, not just the number, was built into the eval from the very first version. Baseline: Faithfulness 2.20, Relevance 2.30, with 5 complete retrieval failures (roughly a quarter of the set returning "I don't have information about that"). 
...
**The limitation that motivated the triad:** the faithfulness prompt conflated two reference points in one rubric — it asked both whether the answer stuck to the ideal answer and whether every claim was supported by the retrieved context, in the same breath. With the judge serving two masters, a low score still couldn't tell you which stage failed — a retrieval failure and a generation failure produced an identical low number. The triad's contribution was to disentangle the two: groundedness grades against the retrieved chunks only, answer relevance against the ideal answer only, so a low score now localizes to a stage. That separation is what the RAG triad was built to provide.
