...
**Groundedness now gates on entities and relations, not just overall support.** The original rubric asked one holistic question, "was the response generated from the documents," and that let a fluent answer built on a name the documents never contained score a 3. The rubric now runs ordered checks first: is the entity the question asked about present in the documents; is every entity the response names present; is every *relation* it asserts (X upgrades to Y, X replaces Y) actually stated, rather than merely inferable from both entities appearing separately. Only after those pass does it weigh the remaining detail. Groundedness also receives the question now, which it previously did not — "is the requested entity missing" is unanswerable without it.

The eval set grew from 15 questions to 20. Four additions are **corpus-divergent stats**, where the BBG value disagrees with the vanilla value a model knows from training (Warrior costs 20 not 40, Scout 15 not 30, Eagle Warrior 32 not 65, Knight melee 50 not 48). These previously existed only as a standalone probe script, which meant the one property that discriminates retrieval from prior was absent from the scored set. The pipeline grounds all four to the corpus. The fifth addition asks what the Eagle Warrior upgrades to, which the corpus cannot answer at all.

Current scores: **CR 2.90 / G 2.90 / AR 2.95 at n=20.** Not comparable to the previous 3.00 / 2.73-2.93 / 2.93 at n=15 — different questions and a different rubric.

**What the upgrade-path question exposed.** The pipeline answered it by naming the gap and then filling it from training data anyway ("the search results don't state the upgrade path… based on standard Civilization 6 mechanics, typically the **Swordsman**, but I cannot confirm this"). Context relevance scored it 1 and said retrieval cannot answer the question, which is the correct and loudest signal. Groundedness scored it 2 — the judge's reasoning names the unsupported relation explicitly and then declines to apply the rubric's score-1 gate, because the response *is* mostly supported and the proportional scale outweighs the categorical rule. Sharpening the wording twice did not move it. That disagreement is left standing rather than forced: an eval that scores a hedged answer identically to a flat fabrication loses a distinction worth keeping, and context relevance already carries the signal.

### Whether the judge flatters its own model

The judge and the generation model were both Sonnet 4.6, so the model graded its own output and nothing measured what that was worth. Testing it required a change to the harness: the eval regenerates answers on every run and generation is not temperature-pinned, so swapping judges and re-running would have confounded the judge with run-to-run variance. The runner now saves each answer and its retrieved documents, and a `--rejudge` pass scores saved answers without regenerating — one variable, and cheaper than a second full run because no pipeline calls are paid twice.

Re-judging the same 20 answers with Haiku 4.5: **16 of 20 rows scored identically**, means moving CR 2.90→2.85, G 2.90→2.80, AR 2.95→2.75. All movement is downward, the direction the bias hypothesis predicts, but the four disagreements do not decompose into bias. One is a judge **error** — Haiku scored a correct answer 1 on answer relevance for being longer than the ideal, which its own prompt explicitly forbids, and removing that single row halves the largest delta. One is Haiku being **more** correct, applying the relation gate on the upgrade-path question where Sonnet overrode it. Two are genuine judgment calls, including whether Qin Shi Huang's two variants are one leader or two.

The claim that survives: **self-evaluation bias is bounded small on this set, not absent.** Broad consistent uplift is what self-grading inflation looks like, and this is four scattered rows with three different causes. One model pair at n=20 does not prove more than that, and because the second judge is weaker its disagreements are ambiguous by construction. Sonnet 4.6 remains the judge of record; the Haiku scores are kept as the comparison.
